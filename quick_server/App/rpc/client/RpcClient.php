...
class RpcClient
{
     ...
     * @param string $pk The pk used for request sign, defaults to an empty string
     * @return instance of RpcClient
     */
    public static function instance($service_name, $pk = '')
    {
        if ($pk != '' and !isset(self::$pks[$service_name])) {
            self::$pks[$service_name] = $pk;
        }
    ...
    public function sendData($method, $arguments)
    {
        ...
        if (isset(self::$pks[$this->serviceName]) and self::$pks[$this->serviceName] != '') {
            $data = $this->makeSign($data, self::$pks[$this->serviceName]);
        }

        if (isset($GLOBALS['app_conf']['rpc_enable_encrypt']) and $GLOBALS['app_conf']['rpc_enable_encrypt'] === true) {
            $data = $this->encryptRPCData($data);
        }
     ...
     * @return void
     */
    public function encryptRPCData(array $data)
    {
        $str = json_encode($data);
        return ['encData' => (new EncryptData(self::$pks[$this->serviceName]))->encrypt($str)];
    }

    /**
     * make a rpc signature
     * @param array $data
     * @param string $pk
     * @return array
     */
    public function makeSign(array $data, $pk)
    {
        $data['nonce'] = microtime(true);
        $data['_pk'] = $pk;
        recursiveKsort($data);
        $data['sign'] = hash('sha256', strtolower(http_build_query($data)));
        unset($data['_pk']);
        return $data;
    }
