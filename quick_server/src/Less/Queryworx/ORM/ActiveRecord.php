...
namespace Less\Queryworx\ORM;

use Less\Queryworx\Base\TableModel;
use Less\Queryworx\Schema\DbSchema;
use Less\Queryworx\Command\DbCriteria;
use Less\Queryworx\Command\CommandBuilder;
use Less\Queryworx\ORM\Relations\BelongsToRelation;
...
{
	const BELONGS_ARRAY = '\Less\Queryworx\ORM\Relations\BelongsArray';
	const BELONGS_TO = '\Less\Queryworx\ORM\Relations\BelongsToRelation';
	const HAS_ONE = '\Less\Queryworx\ORM\Relations\HasOneRelation';
	const HAS_MANY = '\Less\Queryworx\ORM\Relations\HasManyRelation';
	const MANY_MANY = '\Less\Queryworx\ORM\Relations\ManyManyRelation';
	const STAT = '\Less\Queryworx\ORM\Relations\StatRelation';
	...
	/**
	 * The database schema container
	 * @var array
	 */ 
	static public $metaDataContainer = array();
	...
	public function createRelationModel(array $relation)
	{
		...
		$model = new $class($this,$relation[0],$relation[1],$options);
	...
	public function getMetaData($tableName="")
	{
		...
		if(!isset(self::$metaDataContainer[$tableName]))
		{
			self::$metaDataContainer[$tableName] = $this->getSchema($tableName);
		}
		return self::$metaDataContainer[$tableName];
	}
	...
	public function insert($attributes=null)
	{
		...
		if($this->beforeSave())
		{
			...
			throw new \RuntimeException('The method beforeSave must return true.');
		}
		return false;
	}
	...
	public function populateRecord($attributes,$callAfterFind=true)
	{
		if($attributes!==false)
		{
			...
			foreach($attributes as $name=>$value)
			{
				...
				// }
				$record->_oldAttributes[$name] = $value;
			}
